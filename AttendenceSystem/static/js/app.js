        ...
        <div className="animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
                ...
                <div className="glass-card stat-card border-l-4 border-emerald-500">
                    ...
                    <p className="stat-value text-emerald-400">82%</p>
                </div>
                        ...
                        <tbody>
                            {(stats.risky_students || []).map((s, i) => (
                                <tr key={i}>
                                    ...
                                    <td className="p-4"><button className="text-primary hover:underline">Notify Parent</button></td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
...
const AttendanceForm = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (res && res.message) {
            setSuccess(res);
            setFormData({ ...formData, absent_rolls: '' });
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                    ...
                    <div>
                        <label className="block text-sm font-medium mb-2 text-text-muted">
                            Absent Roll Numbers <span className="text-xs italic">(Separated by commas, e.g. 101, 105, 110)</span>
                        </label>
                        <textarea 
                            className="w-full glass-input h-32"
                            placeholder="Example: 102, 108, 125..."
...
const StudentList = () => {
    ...
    const filtered = students.filter(s => 
        ...
        (s.roll_number || "").indexOf(search) !== -1
    );
...
const NotificationLog = () => {
    ...
    const getTypeColor = (type) => {
        if (type === 'Alert') return 'bg-blue-500/20 text-blue-400';
                ...
                {logs.map(log => (
                    <div key={log.id} className="glass-card p-6 flex gap-6 items-start">
                        <div className={"shrink-0 w-12 h-12 rounded-xl flex items-center justify-center " + getTypeColor(log.type)}>
                            <Icon name={log.type === 'Alert' ? 'message-square' : 'alert-triangle'} />
                        </div>
                ...
                <div className="glass-card p-10 max-w-2xl mx-auto text-center text-white">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-left">
                        <div className="p-4 bg-white/5 rounded-xl border border-glass-border">
                            ...
                            <p className="font-semibold">TCH-2024-089</p>
                        </div>
                        <div className="p-4 bg-white/5 rounded-xl border border-glass-border">
                            <p className="text-xs text-text-muted uppercase">Email</p>
                            <p className="font-semibold">sharma.prof@college.edu</p>
                        </div>
                    </div>
                </div>
            );
